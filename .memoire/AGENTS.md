...
"team on standby" — invoked by the `/challenge` flow or directly.
...
Output: shadcn `add` commands + a single React composition in `src/app/challenge/page.tsx`.
...
Owns design tokens when a challenge demands a custom palette. Prefer extending CSS
variables in `src/app/globals.css` over inventing a new system.

## (reserved) dataviz-specialist
Owns chart specs when a challenge involves data. Prefer Recharts; reach for D3 only
